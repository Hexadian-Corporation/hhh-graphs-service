...
class TestDeepHierarchy:
    async def test_two_level_hierarchy(self, mock_maps_client: AsyncMock) -> None:
        ...
        result = await build_tree("a18-uuid", mock_maps_client)

        assert result == ["a18-uuid", "arc-uuid"]
        mock_maps_client.get_location_ancestors.assert_called_once_with("a18-uuid")

    async def test_three_level_hierarchy(self, mock_maps_client: AsyncMock) -> None:
        mock_maps_client.get_location_ancestors.return_value = [
        ...
        result = await build_tree("shop-uuid", mock_maps_client)

        assert result == ["shop-uuid", "a18-uuid", "arc-uuid"]


class TestDirectChildOfStar:
    async def test_lagrange_station(self, mock_maps_client: AsyncMock) -> None:
        mock_maps_client.get_location_ancestors.return_value = [
            LocationData(id="arcl1-uuid", name="ARC-L1", location_type="station", parent_id="stan-uuid"),
        ]

        result = await build_tree("arcl1-uuid", mock_maps_client)

        assert result == ["arcl1-uuid"]

    async def test_gateway(self, mock_maps_client: AsyncMock) -> None:
        mock_maps_client.get_location_ancestors.return_value = [
            LocationData(id="sp-gw-uuid", name="Stanton-Pyro GW", location_type="gateway", parent_id="stan-uuid"),
        ]

        result = await build_tree("sp-gw-uuid", mock_maps_client)

        assert result == ["sp-gw-uuid"]


class TestStarItself:
    async def test_star_returns_empty(self, mock_maps_client: AsyncMock) -> None:
        mock_maps_client.get_location_ancestors.return_value = []

        result = await build_tree("stan-uuid", mock_maps_client)

        assert result == []


class TestSystemFiltering:
    async def test_system_node_excluded_if_present(self, mock_maps_client: AsyncMock) -> None:
    ...
    async def test_node_with_none_parent_excluded(self, mock_maps_client: AsyncMock) -> None:
...
class TestInputIsFirstElement:
    async def test_first_element_is_input_location(self, mock_maps_client: AsyncMock) -> None:
        mock_maps_client.get_location_ancestors.return_value = [
            LocationData(id="a18-uuid", name="Area 18", location_type="city", parent_id="arc-uuid"),
            LocationData(id="arc-uuid", name="ArcCorp", location_type="planet", parent_id="stan-uuid"),
        ]
